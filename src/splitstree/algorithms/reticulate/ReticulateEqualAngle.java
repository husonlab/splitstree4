...
public class ReticulateEqualAngle implements Reticulate2Network {
    ...
    public Network apply(Document doc, Taxa taxa, Reticulate ret) throws Exception {
     ...
     /*
        ...
        Iterator itO = depRet.nodeIterator();
        ...
        while (it.hasNext()) {
            ...
            if (n.getInDegree() == 2) {
                ...
                if (itPath.hasNext())
                    ...
                    if(verbose)  System.out.println("path to short!"); // should contain at least one more node
    ...
    private void recTopDownLabelNodes(PhyloGraphView graphView, PhyloGraph graph, Node start, double startAngle, int nTax, HashMap node2AngleSize, HashMap node2SubNodes, HashSet before, HashMap parent2rNodes, HashMap node2ActiverNodes) throws Exception {
       if(verbose) System.out.println("\nstart: " + start + "\t before: " + before);
        ...
        if (parent2rNodes.get(start) != null) {
            if(verbose) System.out.println("rNodes of parent: " + start + "\tare:" + parent2rNodes.get(start) + "\t  active rNodes: " + node2ActiverNodes.get(start));
        ...
        // some system out
        if(verbose) { for (int i = 0; i < nodes2nConections.length; i++) {
    ...
    private ArrayList buildOrdListWithBranchandBound(ArrayList orderedRNodes, HashSet before, int[][] nodes2nConections, ArrayList decendants, PhyloGraph ordGraph) {
        ...
        if (bound == 0)
            ...
            /*
             // find the root
            Iterator it = ordGraph.nodeIterator();
            Node root = null;
    ...
    private ArrayList recGenerateOrderings(int orderedRNodesIndex, ArrayList orderedRNodes, ArrayList decendants, int[][] nodes2nConections, ArrayList partialOrdering) {
        ...
        if (!node2Pathp.equals(rNodeData.getParent()) && ordIdp == -1) {
            ...
            if(verbose)  System.out.println("ordIdp: " + ordIdp + "\tordIdq: " + ordIdq + "\tstartPosition: " + startPosition + "\tstopPosition: " + stopPosition);
    ...
    private int buildOrdListGreedy(ArrayList orderedRNodes, HashSet before, int[][] nodes2nConections, ArrayList decendants, ArrayList ordList) {
        ...
        while (it.hasNext()) {
            ...
            if (!node2Pathp.equals(rNodeData.getParent()) && ordList.indexOf(node2Pathp) == -1) {
                if(verbose) System.out.println("adding node: " + node2Pathp + "\t before: " + before.contains(node2Pathp));
                int minPosition = 0;
                ...
                if(verbose) System.out.println("minScore: " + minScore + "\tminPosition: " + minPosition + "\tordList: " + ordList);
            }
            if (!node2Pathq.equals(rNodeData.getParent()) && ordList.indexOf(node2Pathq) == -1) {
                if(verbose) System.out.println("adding edge: node " + node2Pathq + "\t before: " + before.contains(node2Pathq));
                int minPosition = 0;
                int minScore = Integer.MAX_VALUE;
                for (int i = 0; i <= ordList.size() && minScore > 0; i++) {
            ...
            if (!ordList.contains(rNode)) {
                ...
                int minPosition = 0;
                int minScore = Integer.MAX_VALUE;
                if(verbose) System.out.println("adding edge: rNode " + rNode + "\t between: " + (startIndex + 1) + "\t" + stopIndex);
        ...
        for (int i = 0; i < ordList.size(); i++) {
            ...
            int index1 = decendants.indexOf(n1);
                for (int j = i + 1; j < ordList.size(); j++) {
                    ...
                    if(verbose) System.out.println("indexP: "+indexP+"\t indexQ: "+indexQ+ "\t index1: "+index1+ "\tindex2: "+index2+"\tmatrixValue: "+nodes2nConections[index1][index2]);
                    //if (j != position && ((indexP != index1 && indexQ != index2) && (indexP != index2 && indexQ != index1))) {
                        score += (j - i - 1) * nodes2nConections[index1][index2];
                        if(verbose) System.out.println("i: " + i + "\tj: " + j + "\tscore: " + score + "\tadded: " + ((j - i - 1) * nodes2nConections[index1][index2]) + "orderedList: " + ordList);
                    //}
                    
            }
            // adding score for before
            score += (i) * nodes2nConections[index1][nodes2nConections.length - 2];
            if(verbose) System.out.println("i: " + i + "\tbefore" + "\tscore: " + score + "\tentry in scoring matrix: " + nodes2nConections[index1][nodes2nConections.length - 2] + "orderedList: " + ordList);

            // adding score for after
            score += (ordList.size() - i) * nodes2nConections[index1][nodes2nConections.length - 1];
            if(verbose) System.out.println("i: " + i + "\tafter" + "\tscore: " + score + "\tentry in scoring matrix: " + nodes2nConections[index1][nodes2nConections.length - 1] + "orderedList: " + ordList);
    ...
    private ArrayList buildLocalWorkFlow(HashSet rNodes, PhyloGraph ordGraph) {
        ...
        /*   System.out.println("local ordering Graph: ");
         Iterator itO = ordGraph.nodeIterator();
         while (itO.hasNext()) System.out.println("Node: " + itO.next());
         System.out.println("-----------------------------------------");
