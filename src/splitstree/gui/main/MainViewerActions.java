...
import splitstree.analysis.bootstrap.TestTreeness;
import splitstree.analysis.characters.CaptureRecapture;
import splitstree.analysis.characters.GascuelGamma;
import splitstree.analysis.characters.PhiTest;
import splitstree.analysis.distances.DeltaScore;
import splitstree.analysis.network.MidpointRoot;
import splitstree.analysis.splits.PhylogeneticDiversity;
import splitstree.core.Document;
import splitstree.core.TaxaSet;
import splitstree.externalIO.imports.ImportManager;
import splitstree.externalIO.imports.NewickTree;
import splitstree.gui.Director;
import splitstree.gui.DirectorActions;
import splitstree.gui.TreesNameDialog;
import splitstree.gui.algorithms.AlgorithmsWindow;
import splitstree.gui.confidence.ConfidenceWindow;
import splitstree.gui.formatter.Formatter;
import splitstree.gui.input.InputDialog;
import splitstree.gui.nodeEdge.Configurator;
    ...
    public void updateEnableState() {
        ...
        for (Action action : all) {
            ...
            if (action.getValue(DEPENDS_ON_EDGESELECTION) != null && viewer.getNumberSelectedEdges() == 0)
                action.setEnabled(false);
            //       if(action.getValue(DEPENDS_ON_ROOTED)!=null && (doc.getNetwork()==null || doc.getAssumptions().getSplitsTransformName()!=Basic.getShortName(RootedEqualAngle.class)))
            //              action.setEnabled(false);
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                if (doc.isValidByName(Trees.NAME) && doc.isValidByName(Assumptions.NAME)) {
                    ...
                    if (treesTransform instanceof TreeSelector) {
                        int which;
                        if (event != null && (event.getModifiers() & KeyEvent.SHIFT_MASK) != 0) {
                            which = 1;
                        } else {
