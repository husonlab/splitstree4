/*
 * BunemanQuartets.java Copyright (C) 2022 Daniel H. Huson
 *
...
/**
 * returns all quartets that have positive buneman index
 */
public class BunemanQuartets implements Distances2Quartets {

    private double threshold = 0;
    public final static String DESCRIPTION = "Computes all quartets with positive Buneman index";
    ...
    public Quartets apply(Document doc, Taxa taxa, Distances distances) throws CanceledException {
        ...
        // ProgressDialog pd = new ProgressDialog("Buneman Quartets...",""); //Set new progress bar.
                ...
                for (int k = 1; k <= taxa.getNtax(); k++)
                    if (k != i && k != j) {
                        for (int m = k; m <= taxa.getNtax(); m++) {
                            double alpha = BunemanTree.getIsolationIndex(i, j, k, m, distances);
                            if (alpha > threshold) {
...
//******************************************
            ...
            //while(true)if(ti+200<=System.currentTimeMillis())break;
//******************************************
