/*
 * DynamicCholesky.java Copyright (C) 2022 Daniel H. Huson
 *
...
public class DynamicCholesky implements Serializable {
  ...
  * ------------------------ */
     ...
     *
     * @param rowToUnmask The row to add to the matrix (i.e. remove zero constraint)
     */

    public void unmaskRow(int rowToUnmask) {
        if (getmask(rowToUnmask)) {
            int newrow = ncurr;
            ncurr = ncurr + 1;
            full2partial[rowToUnmask] = newrow;
            partial2full[newrow] = rowToUnmask;
            mask.set(rowToUnmask, false);

            /* Update the A matrix part of L... copy these from the full matrix */
            for (int i = 0; i < newrow; i++)
                L[i][newrow] = Lfull[partial2full[i]][rowToUnmask];
    ...
    //TODO: Make a JUnit class for DynamicCholesky   (Do this before using this class!!!)
